...
export class MessageService {
    ...
    async setMessageSeenForSingleChat(userId: string, roomId: string): Promise<any> {
        return this.messageModel.updateMany({
            $and: [
                {rId: new mongoose.Types.ObjectId(roomId)},
                {sId: {$ne: new mongoose.Types.ObjectId(userId)}},
                {sAt: {$eq: null}}
            ]
        }, {
            sAt: new Date()
    ...
    async setMessageSeenForGroupChat(userId: string, roomId: string) {
        // let messagesToSeen = await this.groupMessageStatusService.findAll({
        //     $and: [
        //         {rId: roomId},
        //         {uId: {$eq: userId}},
        //         {sAt: {$eq: null}}
    ...
    async setMessageDeliverForGroupChat(myUser: IUser, roomId: string): Promise<boolean> {
        // let messagesNeedToDeliver = await this.groupMessageStatusService.findAll({
        //         $and: [
        //             {rId: roomId},
        //             {uId: {$eq: myUser._id}},
        //             {dAt: {$eq: null}}
