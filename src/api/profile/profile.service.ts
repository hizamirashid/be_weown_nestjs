...
export class ProfileService {
    ...
    async getPeerProfile(dto: MongoIdDto) {
        let res = {};
        let user = await this.userService.findByIdOrThrow(dto.id, "userImage fullName phone userPrivacy bio lastSeenAt createdAt hasBadge");
    ...
    async addPushKey(myUser: IUser, pushKey: String) {
    ...
    async updateMyPassword(dto: UpdateMyPasswordDto) {
        let foundedUser: IUser = await this.userService.findById(
            dto.myUser._id,
            "+password userDevice lastMail banTo phone registerStatus"
        );
    ...
    async checkPassword(user: IUser, password: string) {
        ...
        let foundedUser: IUser = await this.userService.findByIdOrThrow(
            user._id,
            "+password userDevice lastMail banTo phone registerStatus deletedAt"
        );
