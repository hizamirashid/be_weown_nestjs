...
export class UserService extends BaseService<IUser> {
    ...
    async findOneByEmail(phone: string, select?: string) {
        return this.model.findOne({phone: phone}).select(select).lean();
    }

    async findOneByEmailOrThrow(phone: string, select: string) {
        let usr = await this.model.findOne({phone: phone}).select(select).lean();
        if (!usr)
